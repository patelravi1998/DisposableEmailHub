...
export const LeadsPage = () => {
  ...
  const handleExport = () => {
    ...
    const rows = data.map(item => 
      columns.map(col => {
        if (col.accessor === 'is_whatsapp_number_same' || col.accessor === 'willing_to_relocate') {
          return item[col.accessor] === 1 ? 'Yes' : 'No';
        }
        if (col.accessor === 'call_status') {
          return callStatusOptions.find(opt => opt.value === item[col.accessor])?.label || 'Unknown';
        }
        return item[col.accessor] || '';
      })
    );
    
    exportToCsv('employer_details.csv', headers, rows);
  };
    ...
    <div className="p-2 sm:p-4 bg-gray-50 min-h-screen">
      <div className="bg-white rounded-lg shadow-md p-2 sm:p-4">
        <div className="flex flex-col gap-3 mb-4">
          <h2 className="text-lg sm:text-xl font-semibold text-gray-800 text-center sm:text-left">Employer Details</h2>
      ...
      {showModal && (
        <Modal 
          ...
          size="xl"
        >
          <div className="space-y-4 overflow-x-auto max-h-[80vh] overflow-y-auto">
